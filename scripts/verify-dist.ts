...
async function verify(): Promise<void> {
  ...
  if (outputStats.size <= Buffer.byteLength(legacy)) {
    throw new Error("Output is unexpectedly smaller than the Golden Reference");
  }

  console.log(
    `Verified ${outputPath}: metadata, single-file bundle, and byte-identical Legacy body`,
  );
}
